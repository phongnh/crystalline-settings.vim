...
function! s:GetGitBranch() abort
    ...
    if exists('*FugitiveHead')
        ...
        try
            ...
            let l:branch = crystalline_settings#Trim(l:branch)
        finally
...
function! s:TruncateBranch(branch, length) abort
    return strcharpart(a:branch, 0, a:length - 1) .. g:crystalline_symbols.ellipsis
endfunction
...
function! s:SplitBranch(branch, length) abort
    ...
    return l:truncated_branch .. g:crystalline_symbols.ellipsis
endfunction
...
function! s:FormatBranch(branch) abort
    let l:winwidth = crystalline_settings#GetWinWidth(0)

    if l:winwidth >= g:crystalline_winwidth_config.normal
