...
def GetGitBranch(): string
    if has_key(b:, 'crystalline_git_branch') && reltimefloat(reltime(git_branch_time)) < git_branch_expiry
    ...
    # Caching
    b:crystalline_git_branch = branch
...
def TruncateBranch(branch: string, length: number): string
    return strcharpart(branch, 0, length - 1) .. g:crystalline_symbols.ellipsis
enddef
...
def SplitBranch(branch: string, length: number): string
    ...
    return truncated_branch .. g:crystalline_symbols.ellipsis
enddef
...
def FormatBranch(branch: string): string
    const winwidth = crystalline_settings#GetWinWidth(0)

    if winwidth >= g:crystalline_winwidth_config.normal
